...
mwhs_obis_species <- read.csv("species_lists/mwhs_species_list.csv") %>%
  dplyr::rename(c("category" = "redlist_category")) %>%
...
```{r}
webr::PieDonut(species, aes(group), r0 = 0.6, r1 = 1, showRatioThreshold = 1) +
...
#       )
#     ) %>%
#     group_by(group, materialSampleID) %>%
